...
class HumanButton(Button):
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(170, 67, 64), (176, 139, 137)], True)

    def click(self, game):
        if self.enable:
            game.start()
            game.winner = None
            game.play_entrance = game.human_play
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            return True
        return False
...
class MachineButton(Button):
    def __init__(self, screen, text, x, y):
        super().__init__(screen, text, x, y, [(130, 67, 64), (136, 139, 137)], True)

    def click(self, game):
        if self.enable:
            game.start()
            game.winner = None
            game.play_entrance = game.auto_play
            self.msg_image = self.font.render(self.text, True, self.text_color, self.button_color[1])
            self.enable = False
            return True
        return False
...
class Game():
    def __init__(self, caption):
        ...
        self.buttons.append(MachineButton(self.screen, 'machine-run', MAP_WIDTH + 30, BUTTON_HEIGHT + 125))
        self.buttons.append(HumanButton(self.screen, 'Human-PK', MAP_WIDTH + 30, BUTTON_HEIGHT + 205))
        self.is_play = False
        self.play_entrance = self.normal_play
    ...
    def human_play(self):
        ...
        if self.is_play and not self.isOver():
            ...
            if self.action is not None:
                self.checkClick(self.action[0], self.action[1], True)
                self.action = None
